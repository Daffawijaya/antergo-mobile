...
export const DRIVER_BACKGROUND_LOCATION_TASK = 'ANTERGO_DRIVER_BACKGROUND_LOCATION';
...
async function readTrackingMode(): Promise<DriverTrackingMode> {
  ...
  return (await SecureStore.getItemAsync(TRACKING_MODE_KEY) as DriverTrackingMode | null) ?? 'stopped';
}

export async function setDriverTrackingMode(mode: DriverTrackingMode) {
  if (Platform.OS !== 'web') await SecureStore.setItemAsync(TRACKING_MODE_KEY, mode);
}

TaskManager.defineTask<{ locations: Location.LocationObject[] }>(DRIVER_BACKGROUND_LOCATION_TASK, async ({ data, error }) => {
  if (error || backgroundRequestInFlight || await readTrackingMode() !== 'background') return;
  ...
  try {
    ...
    if (isAxiosError(requestError) && requestError.response?.status === 401) await stopDriverLocationTracking();
  } finally {
...
export async function startDriverBackgroundTracking() {
  if (Platform.OS === 'web') throw new Error('Background location tidak tersedia di web.');
  if (!await TaskManager.isAvailableAsync()) throw new Error('Background tracking memerlukan Expo Development Build di perangkat fisik.');
  ...
  await Location.startLocationUpdatesAsync(DRIVER_BACKGROUND_LOCATION_TASK, {
    ...
    foregroundService: {
      ...
      notificationBody: 'AnterGo sedang menggunakan lokasi untuk menerima dan menjalankan pesanan.',
