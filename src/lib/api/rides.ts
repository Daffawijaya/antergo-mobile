import type { CancelRideResponse, CreateRideInput, CreateRideResponse, LaravelPaginator, Order, OrderDetailResponse } from '@/types/api';
...
export async function listCustomerOrders(page = 1) {
  return (await apiClient.get<LaravelPaginator<Order>>('/orders', { params: { page } })).data;
}
...
export async function cancelRide(id: number, cancelledReason?: string) {
  return (await apiClient.post<CancelRideResponse>(`/orders/${id}/cancel`, {
