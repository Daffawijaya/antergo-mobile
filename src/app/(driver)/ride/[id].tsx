...
const TRANSITIONS: Partial<Record<OrderStatus, { status: DriverRideStatusUpdate; label: string }>> = {
  driver_assigned: { status: 'driver_arrived', label: 'Sudah Sampai Lokasi Jemput' },
...
export default function DriverRideDetailScreen() {
  ...
  const detail = useQuery({
    ...
    enabled: validId,
    refetchInterval: ({ state }) => state.data && TERMINAL.has(state.data.status) ? false : 5_000,
  });
  const transition = useMutation({
    mutationFn: ({ status }: { status: DriverRideStatusUpdate }) => updateRideStatus(orderId, status),
  ...
  return <Screen>
    ...
    <PageHeader eyebrow="Driver Ride" title={detail.data?.order_number ?? 'Detail Ride'} description="Status customer dan driver disinkronkan melalui Laravel API." />
    {detail.isLoading ? <StatusState type="loading" /> : detail.isError ? <StatusState type="error" message={getApiErrorMessage(detail.error)} action={<Button title="Coba lagi" variant="secondary" onPress={() => detail.refetch()} />} /> : detail.data ? <>
      <Card>
      ...
      <RideMap
        pickup={parseCoordinate(detail.data.pickup_latitude, detail.data.pickup_longitude)}
        destination={parseCoordinate(detail.data.destination_latitude, detail.data.destination_longitude)}
        driver={parseCoordinate(detail.data.driver?.location?.latitude, detail.data.driver?.location?.longitude)}
        focus={detail.data.status === 'in_progress' || detail.data.status === 'completed' ? 'destination' : 'pickup'}
      />
      <Card>
        <Text style={styles.sectionTitle}>Rute</Text>
        <View style={styles.route}><Text style={styles.routeLabel}>Jemput</Text><Text style={styles.body}>{detail.data.pickup_address ?? '-'}</Text></View>
        <View style={styles.route}><Text style={styles.routeLabel}>Tujuan</Text><Text style={styles.body}>{detail.data.destination_address ?? '-'}</Text></View>
        <KeyValue label="Catatan" value={detail.data.notes || '-'} />
      </Card>
      {detail.data.driver?.vehicle ? <Card>
        <Text style={styles.sectionTitle}>Kendaraan</Text>
        <KeyValue label="Kendaraan" value={`${detail.data.driver.vehicle.brand} ${detail.data.driver.vehicle.model}`} />
        <KeyValue label="Plat nomor" value={detail.data.driver.vehicle.plate_number} />
        <KeyValue label="Warna" value={detail.data.driver.vehicle.color} />
      </Card> : null}
      <Card>
...
const styles = StyleSheet.create({ sectionTitle: { color: Colors.text, fontSize: 18, fontWeight: '800' }, route: { gap: 3 }, routeLabel: { color: Colors.primary, fontSize: 12, fontWeight: '800', textTransform: 'uppercase' }, body: { color: Colors.text, lineHeight: 21 }, muted: { color: Colors.muted, lineHeight: 20 }, history: { gap: 6, paddingVertical: 8, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: Colors.border }, date: { color: Colors.muted, fontSize: 12 }, error: { color: Colors.danger, lineHeight: 20 } });
