...
export default function MerchantListScreen() {
  ...
  const title = service === "shopping" ? "Shopping" : "Food";
  const [page, setPage] = useState(1);
  const query = useQuery({
    queryKey: [...foodKeys.merchants, service, page],
    queryFn: () => listMerchants(page, service),
  ...
  return (
    <Screen>
      ...
      <PageHeader
        ...
        title={
          service === "shopping" ? "Belanja produk lokal" : "Makanan & minuman"
        }
        description="Pilih merchant UMKM yang sedang buka."
      />
      {query.isLoading ? (
        ...
        <StatusState
          type="empty"
          message="Belum ada merchant yang sedang buka."
        />
      ) : (
        <>
          {query.data.data.map((merchant) => (
            <Pressable
              ...
              onPress={() =>
                router.push({
                  ...
                  params: { id: String(merchant.id), service },
                })
              }
            ...
            >
              <Card>
                ...
                <KeyValue
                  label="Status"
                  value={merchant.is_open ? "Buka" : "Tutup"}
                />
