...
export default function RideDetailScreen() {
  ...
  return <Screen>
    ...
    <PageHeader eyebrow="Ride Detail" title={query.data?.order_number ?? 'Detail perjalanan'} description="Status diperbarui otomatis selama perjalanan aktif." />
    {query.isLoading ? <StatusState type="loading" /> : query.isError ? <StatusState type="error" message={getApiErrorMessage(query.error)} action={<Button title="Coba lagi" variant="secondary" onPress={() => query.refetch()} />} /> : query.data ? <>
      ...
      <RideMap
        ...
        driver={parseCoordinate(query.data.driver?.location?.latitude, query.data.driver?.location?.longitude)}
        focus="all"
      />
      <Card>
        <Text style={styles.sectionTitle}>Rute</Text>
        <View style={styles.routeItem}><Text style={styles.routeLabel}>Jemput</Text><Text style={styles.routeText}>{query.data.pickup_address ?? '-'}</Text></View>
        <View style={styles.routeItem}><Text style={styles.routeLabel}>Tujuan</Text><Text style={styles.routeText}>{query.data.destination_address ?? '-'}</Text></View>
        <KeyValue label="Catatan" value={query.data.notes || '-'} />
      </Card>
      {query.data.driver ? <Card>
        ...
        {query.data.driver.vehicle ? <>
          <KeyValue label="Kendaraan" value={`${query.data.driver.vehicle.brand} ${query.data.driver.vehicle.model}`} />
          <KeyValue label="Plat nomor" value={query.data.driver.vehicle.plate_number} />
          <KeyValue label="Warna" value={query.data.driver.vehicle.color} />
        </> : <Text style={styles.muted}>Data kendaraan belum tersedia.</Text>}
      </Card> : query.data.status === 'searching_driver' ? <Card><Text style={styles.muted}>Sedang mencari driver terdekat.</Text></Card> : null}
      <Card>
      ...
      {CANCELLABLE_STATUSES.has(query.data.status) ? <Card>
        ...
        {cancelMutation.isError ? <Text style={styles.error}>{getApiErrorMessage(cancelMutation.error)}</Text> : null}
        <Button title="Batalkan Ride" variant="danger" loading={cancelMutation.isPending} onPress={() => cancelMutation.mutate()} />
      </Card> : null}
...
const styles = StyleSheet.create({
  ...
  routeItem: { gap: 3 }, routeLabel: { color: Colors.primary, fontSize: 12, fontWeight: '800', textTransform: 'uppercase' }, routeText: { color: Colors.text, lineHeight: 21 },
  muted: { color: Colors.muted, lineHeight: 20 }, history: { gap: 6, paddingVertical: 8, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: Colors.border }, historyDate: { color: Colors.muted, fontSize: 12 }, error: { color: Colors.danger, lineHeight: 20 },
});
