...
const JAKARTA = { latitude: -6.2, longitude: 106.816666, latitudeDelta: .08, longitudeDelta: .08 };

export function RideMap({ pickup, destination, driver, onMapPress, onPickupChange, onDestinationChange, showsUserLocation, focus = 'all' }: Props) {
  const ref = useRef<MapView>(null);
  useEffect(() => {
    const coordinates = focus === 'pickup' ? [driver, pickup] : focus === 'destination' ? [driver, destination] : [pickup, destination, driver];
    const valid = coordinates.filter((item): item is Coordinate => !!item);
    if (valid.length >= 2) ref.current?.fitToCoordinates(valid, { animated: true, edgePadding: { top: 45, right: 45, bottom: 45, left: 45 } });
    else if (valid[0]) ref.current?.animateToRegion({ ...valid[0], latitudeDelta: .025, longitudeDelta: .025 }, 400);
